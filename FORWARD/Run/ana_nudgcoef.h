...
! North
      DO j=MAX(JstrR,Mm(ng)+1-Iwrk),JendR
        DO i=IstrR,IendR
          wrk(i,j)=cff1*(1.0_r8+COS(pi*REAL(Mm(ng)+1-j,r8)/            &
     &				    REAL(Iwrk,r8)))
        END DO
      END DO
! South
...
#  endif
!-----------Start added by FV------------------
# elif defined NS8KMNOTDEFINEDHERE
      cff1=1.0_r8/(7.0_r8*86400.0_r8)  !Tidsskala (dager) på passiv (outflow)
      cff2=0.0_r8
      DO j=JstrR,JendR
        DO i=IstrR,IendR
          wrk(i,j) = cff2
...
!WEST
      DO j=JstrR,JendR
        DO i=IstrR,20
          wrk(i,j)=cff1*COS((i/9.0_r8)*pi/2.0_r8)
        END DO
      END DO
!EAST
      DO j=JstrR,JendR
        DO i=IendR-20,IendR
          wrk(i,j)=cff1*COS(((IendR-i)/9.0_r8)*pi/2.0_r8)
        END DO
      END DO
!SOUTH
      DO j=JstrR,20
        DO i=IstrR+j,IendR-j
          wrk(i,j)=cff1*COS((j/9.0_r8)*pi/2.0_r8)
        END DO
      END DO
!NORTH
      DO j=JendR-20,JendR
        DO i=IstrR-j+JendR,IendR+j-JendR
          wrk(i,j)=cff1*COS(((JendR-j)/9.0_r8)*pi/2.0_r8)
        END DO
      END DO
      ...
      !DO itrc=1,NT(ng)
        DO j=JstrR,JendR
          DO i=IstrR,IendR
            ...
            CLIMA(ng)%Tnudgcof(i,j,isalt)=Tnudg(isalt,ng)
          END DO
        END DO
      !END DO
#  endif
